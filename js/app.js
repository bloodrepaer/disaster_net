...
=======
const SYNC_API_BASE = sessionStorage.getItem('dnet_sync_api') || 'http://localhost:8787';
>>>>>>> a6cbbca87a639d73b97968ce13e790e76ee2d74f
      ...
      <div style="grid-column:1/-1;padding:14px 24px;border-bottom:1px solid var(--border);background:var(--surface);display:flex;align-items:center;justify-content:space-between;flex-shrink:0">
        <div style="display:flex;align-items:center;gap:12px">
          ...
          <div>
            ...
            <div style="font-family:'JetBrains Mono',monospace;font-size:9px;color:var(--text3);margin-top:2px">${workerName} · ${currentUser?.team || 'Field Unit'} · Maharashtra Flood Relief</div>
          </div>
        </div>
...
function renderRwRescueList() {
  ...
  if (!rescuedPersons.length) {
    el.innerHTML = '<div style="padding:12px;background:var(--surface);border:1px solid var(--border);border-radius:4px;font-family:monospace;font-size:9px;color:var(--text3);text-align:center">No rescues logged yet.</div>';
    return;
  }
  ...
  el.innerHTML = rescuedPersons.map(p => `
    <div style="background:var(--surface);border:1px solid var(--border);border-radius:4px;padding:10px 12px;margin-bottom:6px;animation:msg-in 0.2s ease-out">
      <div style="display:flex;justify-content:space-between;align-items:center;margin-bottom:4px">
        <span style="font-family:'JetBrains Mono',monospace;font-size:9px;color:var(--accent)">${p.id}</span>
        <span style="font-family:'JetBrains Mono',monospace;font-size:8px;color:${condColors[p.initialCondition] || 'var(--text3)'}">● ${(p.initialConditionLabel || p.initialCondition || '').replace(/[🔴🟠🟡🟢✅]\s*/,'')}</span>
      </div>
      <div style="font-size:12px;font-weight:600;color:var(--text);margin-bottom:2px">${p.name}</div>
      <div style="font-family:'JetBrains Mono',monospace;font-size:9px;color:var(--text3)">Zone ${p.zone} · ${p.rescueTime} · By: ${p.rescuerName}</div>
      ${p.notes ? `<div style="font-size:10px;color:var(--text2);margin-top:4px">${p.notes}</div>` : ''}
    </div>`).join('');
}
  ...
  container.innerHTML = `
    <div style="display:grid;grid-template-columns:1fr 1fr;grid-template-rows:auto 1fr;gap:0;height:calc(100vh - 52px - 28px);overflow:hidden">
      ...
      <div style="grid-column:1/-1;padding:14px 24px;border-bottom:1px solid var(--border);background:var(--surface);display:flex;align-items:center;justify-content:space-between">
        <div style="display:flex;align-items:center;gap:12px">
          ...
          <div>
            ...
            <div style="font-family:'JetBrains Mono',monospace;font-size:9px;color:var(--text3);margin-top:2px">${staffName} · ${facility} · Maharashtra Flood Relief</div>
          </div>
        </div>
        <!-- Stats strip -->
        <div style="display:flex;gap:16px">
          <div style="text-align:center">
            ...
            <div style="font-family:'JetBrains Mono',monospace;font-size:8px;color:var(--text3)">SAVED</div>
          </div>
          <div style="text-align:center">
            <div style="font-family:'Orbitron',monospace;font-size:18px;font-weight:700;color:var(--red)" id="hosp-deceased">${deceased}</div>
            <div style="font-family:'JetBrains Mono',monospace;font-size:8px;color:var(--text3)">DECEASED</div>
          </div>
          <div style="text-align:center">
            <div style="font-family:'Orbitron',monospace;font-size:18px;font-weight:700;color:var(--warn)" id="hosp-treating">${treating}</div>
            <div style="font-family:'JetBrains Mono',monospace;font-size:8px;color:var(--text3)">AWAITING</div>
          </div>
        </div>
      </div>
      ...
      <div style="grid-column:1;border-right:1px solid var(--border);overflow-y:auto;display:flex;flex-direction:column">
        ...
        <!-- Incoming patients -->
        <div style="padding:16px;border-bottom:1px solid var(--border)">
          <div style="font-family:'JetBrains Mono',monospace;font-size:9px;color:var(--text3);letter-spacing:2px;margin-bottom:10px">🚑 INCOMING PATIENTS — LOG TREATMENT</div>
        ...
        <div style="padding:16px">
          <div style="font-family:'JetBrains Mono',monospace;font-size:9px;color:var(--text3);letter-spacing:2px;margin-bottom:10px">📋 UPDATE TREATMENT RECORD</div>
          <div style="display:flex;flex-direction:column;gap:6px">
            ...
            <textarea class="mp-input" id="hosp_notes" placeholder="Treatment notes: procedures, medications, prognosis..." style="resize:none;height:52px;font-size:11px"></textarea>
            <button class="btn btn-primary" style="width:100%;font-size:10px" onclick="saveHospTreatment()">💾 SAVE TREATMENT RECORD</button>
          </div>
        </div>
      </div>
      ...
      <div style="grid-column:2;overflow-y:auto;display:flex;flex-direction:column">
        ...
        <!-- Record outcome -->
        <div style="padding:16px;border-bottom:1px solid var(--border)">
          <div style="font-family:'JetBrains Mono',monospace;font-size:9px;color:var(--text3);letter-spacing:2px;margin-bottom:10px">📝 RECORD OUTCOME</div>
          <div style="display:flex;flex-direction:column;gap:6px">
            ...
            <div style="display:grid;grid-template-columns:1fr 1fr;gap:6px">
              <button class="btn" style="background:rgba(57,255,20,0.1);border:1px solid rgba(57,255,20,0.4);color:var(--accent3);font-size:10px;padding:8px" onclick="recordHospOutcome('saved')">✅ SAVED / RECOVERED</button>
              <button class="btn" style="background:rgba(255,23,68,0.1);border:1px solid rgba(255,23,68,0.4);color:var(--red);font-size:10px;padding:8px" onclick="recordHospOutcome('deceased')">⚰ DECEASED</button>
            </div>
            <textarea class="mp-input" id="hosp_outcome_notes" placeholder="Clinical notes, cause of death, treatment summary..." style="resize:none;height:44px;font-size:11px"></textarea>
          </div>
        </div>
        
        <!-- Outcome log -->
        <div style="padding:16px;flex:1">
          <div style="font-family:'JetBrains Mono',monospace;font-size:9px;color:var(--text3);letter-spacing:2px;margin-bottom:10px">📊 OUTCOME REGISTER — TODAY</div>
...
function renderHospPatientList() {
  ...
  if (!awaiting.length) return '<div style="padding:10px;background:var(--surface);border:1px solid var(--border);border-radius:4px;font-family:monospace;font-size:9px;color:var(--text3);text-align:center">No patients currently awaiting treatment.</div>';
  ...
  return awaiting.map(p => `
    <div style="background:var(--surface);border:1px solid var(--border);border-left:2px solid ${condColors[p.initialCondition] || 'var(--warn)'};border-radius:4px;padding:10px 12px;margin-bottom:6px">
      <div style="display:flex;justify-content:space-between;align-items:center;margin-bottom:3px">
        <span style="font-family:'JetBrains Mono',monospace;font-size:9px;color:var(--accent)">${p.id}</span>
        <span style="font-family:'JetBrains Mono',monospace;font-size:8px;color:${condColors[p.initialCondition] || 'var(--warn)'}">⬤ ${(p.initialConditionLabel||'').replace(/[🔴🟠🟡🟢✅]\s*/,'').toUpperCase()||p.initialCondition.toUpperCase()}</span>
      </div>
      <div style="font-size:12px;font-weight:600;color:var(--text)">${p.name}</div>
      <div style="font-family:'JetBrains Mono',monospace;font-size:9px;color:var(--text3)">Zone ${p.zone} · Age: ${p.age} · ${p.rescueTime}</div>
      ${p.notes ? `<div style="font-size:10px;color:var(--text2);margin-top:4px">${p.notes}</div>` : ''}
    </div>`).join('');
}
...
function renderHospOutcomeLog() {
  ...
  if (!hospitalOutcomes.length) {
    el.innerHTML = '<div style="padding:12px;background:var(--surface);border:1px solid var(--border);border-radius:4px;font-family:monospace;font-size:9px;color:var(--text3);text-align:center">No outcomes recorded yet.</div>';
    return;
  }
  el.innerHTML = hospitalOutcomes.map(o => {
    ...
    return `<div style="background:var(--surface);border:1px solid ${borderCol};border-left:2px solid ${col};border-radius:4px;padding:10px 12px;margin-bottom:6px;animation:msg-in 0.2s ease-out">
      <div style="display:flex;justify-content:space-between;align-items:center;margin-bottom:3px">
        <span style="font-family:'JetBrains Mono',monospace;font-size:8px;color:var(--text3)">${o.id} · ${o.time}</span>
        <span style="font-family:'JetBrains Mono',monospace;font-size:8px;font-weight:700;color:${col}">${isSaved ? '✅ SAVED' : '⚰ DECEASED'}</span>
      </div>
      <div style="font-size:12px;font-weight:600;color:var(--text);margin-bottom:2px">${o.name}</div>
      <div style="font-family:'JetBrains Mono',monospace;font-size:9px;color:var(--text3)">Zone ${o.zone} · Age: ${o.age} · Dr. ${o.doctor}</div>
      ${o.notes ? `<div style="font-size:10px;color:var(--text2);margin-top:4px">${o.notes}</div>` : ''}
    </div>`;
  }).join('');
}
