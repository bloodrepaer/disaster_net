...
window.addEventListener('load', () => {
  // FIX: Set node ID immediately on load
...
// ==============================================================
// ZAMA TFHE — Real FHE encryption
// ==============================================================
async function initTFHE() {
  try {
    const { TfheClientKey, TfheCompactPublicKey, CompactCiphertextList } = await import(
      'https://cdn.jsdelivr.net/npm/tfhe@0.6.4/tfhe.js'
    );
...
// ==============================================================
// STORACHA / IPFS — Real content-addressed storage
// ==============================================================
async function pinToIPFS(data) {
  try {
    ...
    addMeshLog(`📦 IPFS pinned: ${cidStr.substring(0,20)}... — Storacha/Filecoin`);
    return cidStr;
  } catch (e) {
    // Fallback: deterministic CID from content hash
...
function verifyWorld(type) {
  ...
  addHypercert({ name: nearId, action: 'Identity registered on NEAR Protocol', zone: 'ALL', trust: label });
...
function skipVerify() {
  ...
  addLog('system', `Running in demo mode. World ID verification skipped. In production, all volunteers verify via NEAR + World proof-of-personhood to prevent fake volunteer fraud.`);
...
async function submitReport() {
  ...
  // Pin field report to IPFS
  const cid = await pinToIPFS({ report: txt, zone: txt.match(/Zone\s+(\w+)/i)?.[1], ts: Date.now(), node: NODE_ID });

  if (simMode || !apiKey) {
    await simulatedDecision(txt, cid);
  } else {
    await callClaude(txt, cid);
  }
...
async function callClaude(report, cid) {
  try {
    ...
    if (data.content?.[0]) {
      ...
      addLog('decision', decision + `<br><span style="font-family:JetBrains Mono,monospace;font-size:9px;color:var(--text3)">📦 IPFS: ${cid.substring(0,24)}...</span>`);
    ...
    addLog('system', `⚠ Connection error. Falling back to simulation.`);
    simMode = true; updateAIStatus();
    await simulatedDecision(report, cid);
  }
}
...
async function simulatedDecision(txt, cid) {
  ...
  if (lc.includes('oxygen') || lc.includes('hospital') || lc.includes('critical')) {
    resp = `⚡ DECISION: Medical emergency escalated. Triggering FHE-encrypted data share from Zone C3 hospital to Nashik Medical Hub via Lit Protocol key gate — diagnostics accessible to verified physicians only. Helicopter from Zone G1 (low priority) redirected immediately. 3 field medics reassigned from Zone F2. Hypercert minting authorized for responding team.`;
  } else if (lc.includes('bridge') || lc.includes('road') || lc.includes('route') || lc.includes('block')) {
    resp = `⚡ DECISION: Route obstruction confirmed. Diverting Supply Convoy 4 via NH166 alternate — ETA +38min to Zone B1. Flagging route change to all mesh nodes (3 devices confirmed receipt). Zone D1 rescue team redirected to cover supply gap. Starknet contract updated with route deviation log.`;
  } else if (lc.includes('flood') || lc.includes('water') || lc.includes('stranded') || lc.includes('boat')) {
    resp = `⚡ DECISION: Flood rescue activated. Dispatching 3 boats from Zone E4 reserve. Estimated rescue window 3.5hrs before water rises further. NEAR-verified volunteer Squad 7 activated — identity confirmed via World ID. All field logs encrypted and synced to IPFS/Filecoin via Storacha. Hypercert minting queued for Squad 7.`;
  } else if (lc.includes('supply') || lc.includes('food') || lc.includes('medicine')) {
    resp = `⚡ DECISION: Supply shortage logged. Redirecting 8 packages from Zone D1 (22 units available). Estimated delivery 2.3hrs. NGO disbursement contract on Starknet updated. Volunteer contribution logged for Hypercert minting.`;
  } else {
    resp = `⚡ DECISION: Report processed. Severity assessed as HIGH. Nearest available unit — Zone D1 team — dispatched (ETA 18min). Field data encrypted and broadcast to 3 mesh nodes. Resource reallocation complete: 2 medics reassigned.`;
  }
...
async function logMissingPerson() {
  ...
  showToast(`🔒 ${name} logged · FHE encrypted · IPFS pinned`);
}
...
function simulateMeshActivity() {
  const events = [
    ...
    [62000, () => { addMeshLog('💸 Starknet: disbursement condition verified — releasing funds'); }],
  ];
...
setInterval(() => {
  ...
  if (sv) { const sn = parseInt(sv.textContent); if (Math.random() > 0.8) sv.textContent = sn + 1; }
}, 4000);
